...
def run_subprocess(
    ...
    try:
        result = subprocess.run(
            cmd, check=check, capture_output=capture_output, text=True, timeout=300
        )
        return result
...
class ChatMessage(BaseModel):
    ...
    tool_call_id: Optional[str] = Field(
        None, description="ID of tool call being responded to"
    )
...
class ChatCompletionRequest(BaseModel):
    ...
    tool_choice: Optional[Union[str, Dict[str, Any]]] = Field(
        "auto", description="Tool selection strategy"
    )
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        raise HTTPException(
            408, "Request timeout - model may be processing complex request"
        )
    except Exception as e:
